...
/**
 * External mod setask functions unit tests
 *
 * @package mod_setask
 * @category external
 ...
 */
class mod_setask_external_testcase extends externallib_advanced_testcase {
    ...
    protected function setUp() {
        global $CFG;
        require_once($CFG->dirroot . '/mod/setask/externallib.php');
    }
    ...
    public function test_get_grades() {
        ...
        // Create a course and setaskment.
        ...
        $setaskdata['course'] = $course->id;
        $setaskdata['name'] = 'lightwork setaskment';

        $setask = self::getDataGenerator()->create_module('setask', $setaskdata);
        ...
        $roleid = $this->setaskUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($setask->cmid);
        $this->setaskUserCapability('mod/setask:grade', $context->id, $roleid);
        ...
        $submission->setaskment = $setask->id;
        ...
        $DB->insert_record('setask_submission', $submission);

        $grade = new stdClass();
        $grade->setaskment = $setask->id;
        ...
        $DB->insert_record('setask_grades', $grade);
        ...
        $setaskmentids[] = $setask->id;
        $result = mod_setask_external::get_grades($setaskmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_setask_external::get_grades_returns(), $result);

        // Check that the correct grade information for the student is returned.
        $this->assertEquals(1, count($result['setaskments']));
        $setaskment = $result['setaskments'][0];
        $this->assertEquals($setask->id, $setaskment['setaskmentid']);
        // Should only get the last grade for this student.
        $this->assertEquals(1, count($setaskment['grades']));
        $grade = $setaskment['grades'][0];
    ...
    /**
     * Test get_setaskments
     */
    public function test_get_setaskments() {
        global $DB, $USER;
        ...
        // Create the setaskment module.
        $setask1 = self::getDataGenerator()->create_module('setask', array(
            'course' => $course1->id,
            'name' => 'lightwork setaskment',
            'intro' => 'the setaskment intro text here',
            'markingworkflow' => 1,
            'markingallocation' => 1
        ));
        ...
        $roleid = $this->setaskUserCapability('moodle/course:view', $context->id);
        $context = context_module::instance($setask1->cmid);
        $this->setaskUserCapability('mod/setask:view', $context->id, $roleid);
        ...
        // Add a file as setaskment attachment.
        $filerecord = array('component' => 'mod_setask', 'filearea' => ASSIGN_INTROATTACHMENT_FILEAREA,
        ...
        $result = mod_setask_external::get_setaskments();

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_setask_external::get_setaskments_returns(), $result);

        // Check the course and setaskment are returned.
        $this->assertEquals(1, count($result['courses']));
        $course = $result['courses'][0];
        $this->assertEquals('Lightwork Course 1', $course['fullname']);
        $this->assertEquals(1, count($course['setaskments']));
        $setaskment = $course['setaskments'][0];
        $this->assertEquals($setask1->id, $setaskment['id']);
        $this->assertEquals($course1->id, $setaskment['course']);
        $this->assertEquals('lightwork setaskment', $setaskment['name']);
        $this->assertEquals('the setaskment intro text here', $setaskment['intro']);
        $this->assertEquals(1, $setaskment['markingworkflow']);
        $this->assertEquals(1, $setaskment['markingallocation']);

        $this->assertCount(1, $setaskment['introattachments']);
        $this->assertEquals('introattachment.txt', $setaskment['introattachments'][0]['filename']);

        // Now, hide the descritption until the submission from date.
        $DB->set_field('setask', 'alwaysshowdescription', 0, array('id' => $setask1->id));
        $DB->set_field('setask', 'allowsubmissionsfromdate', time() + DAYSECS, array('id' => $setask1->id));

        $result = mod_setask_external::get_setaskments(array($course1->id));
        ...
        $this->assertEquals($course1->id, $setaskment['course']);
        $this->assertEquals('lightwork setaskment', $setaskment['name']);
        $this->assertArrayNotHasKey('intro', $setaskment);
        $this->assertArrayNotHasKey('introattachments', $setaskment);
        $this->assertEquals(1, $setaskment['markingworkflow']);
        $this->assertEquals(1, $setaskment['markingallocation']);

        $result = mod_setask_external::get_setaskments(array($course2->id));
    ...
    public function test_get_submissions() {
        ...
        $setaskdata['course'] = $course1->id;
        $setaskdata['name'] = 'lightwork setaskment';

        $setask1 = self::getDataGenerator()->create_module('setask', $setaskdata);

        // Create a student with an online text submission.
        // First attempt.
        $student = self::getDataGenerator()->create_user();
        $submission = new stdClass();
        $submission->setaskment = $setask1->id;
        $submission->userid = $student->id;
        $submission->timecreated = time();
        ...
        $onlinetextsubmission->setaskment = $setask1->id;
        $DB->insert_record('setasksubmission_onlinetext', $onlinetextsubmission);
        ...
        $setaskmentids[] = $setask1->id;
        $result = mod_setask_external::get_submissions($setaskmentids);
        $result = external_api::clean_returnvalue(mod_setask_external::get_submissions_returns(), $result);

        // Check the online text submission is returned.
        $this->assertEquals(1, count($result['setaskments']));
        $setaskment = $result['setaskments'][0];
        $this->assertEquals($setask1->id, $setaskment['setaskmentid']);
        $this->assertEquals(1, count($setaskment['submissions']));
        $submission = $setaskment['submissions'][0];
    ...
    public function test_get_user_flags() {
        ...
        $DB->insert_record('setask_user_flags', $userflag);

        $setaskmentids[] = $setask->id;
        $result = mod_setask_external::get_user_flags($setaskmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_setask_external::get_user_flags_returns(), $result);
        ...
        $this->assertEquals(1, count($setaskment['userflags']));
        $userflag = $setaskment['userflags'][0];
    ...
    public function test_get_user_mappings() {
        ...
        $roleid = $this->setaskUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($setask->cmid);
        $this->setaskUserCapability('mod/setask:revealidentities', $context->id, $roleid);
        ...
        $mapping->setaskment = $setask->id;
        $mapping->userid = $student->id;

        $DB->insert_record('setask_user_mapping', $mapping);

        $setaskmentids[] = $setask->id;
        $result = mod_setask_external::get_user_mappings($setaskmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_setask_external::get_user_mappings_returns(), $result);
        ...
        $this->assertEquals(1, count($setaskment['mappings']));
        $mapping = $setaskment['mappings'][0];
    ...
    public function test_lock_submissions() {
        ...
        $params['setasksubmission_onlinetext_enabled'] = 1;
        ...
        $submission = $setask->get_user_submission($student1->id, true);
        $data = new stdClass();
        ...
        $result = mod_setask_external::lock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_setask_external::lock_submissions_returns(), $result);

        // Check for 0 warnings.
        $this->assertEquals(0, count($result));

        $this->setUser($student2);
        $submission = $setask->get_user_submission($student2->id, true);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
    ...
    public function test_unlock_submissions() {
        ...
        $result = mod_setask_external::unlock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_setask_external::unlock_submissions_returns(), $result);
    ...
    public function test_submit_for_grading() {
        ...
        set_config('submissionreceipts', 0, 'setask');
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_setask');
        $params['course'] = $course->id;
        $params['setasksubmission_onlinetext_enabled'] = 1;
        ...
        $result = mod_setask_external::submit_for_grading($instance->id, false);
        $result = external_api::clean_returnvalue(mod_setask_external::submit_for_grading_returns(), $result);

        // Should be 1 fail because the submission statement was not aceptted.
        $this->assertEquals(1, count($result));

        $result = mod_setask_external::submit_for_grading($instance->id, true);
        $result = external_api::clean_returnvalue(mod_setask_external::submit_for_grading_returns(), $result);

        // Check for 0 warnings.
        $this->assertEquals(0, count($result));

        $submission = $setask->get_user_submission($student1->id, false);
    ...
    public function test_save_user_extensions() {
        ...
        $this->setUser($teacher);
        $result = mod_setask_external::save_user_extensions($instance->id, array($student1->id), array($now, $tomorrow));
        $result = external_api::clean_returnvalue(mod_setask_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_setask_external::save_user_extensions($instance->id, array($student1->id), array($yesterday - 10));
        $result = external_api::clean_returnvalue(mod_setask_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_setask_external::save_user_extensions($instance->id, array($student1->id), array($tomorrow));
        $result = external_api::clean_returnvalue(mod_setask_external::save_user_extensions_returns(), $result);
        $this->assertEquals(0, count($result));
    ...
    public function test_reveal_identities() {
        ...
        $result = mod_setask_external::reveal_identities($instance->id);
        $result = external_api::clean_returnvalue(mod_setask_external::reveal_identities_returns(), $result);
        $this->assertEquals(1, count($result));
    ...
    public function test_revert_submissions_to_draft() {
        ...
        $this->setUser($teacher);
        $students = array($student1->id, $student2->id);
        $result = mod_setask_external::revert_submissions_to_draft($instance->id, array($student1->id));
        $result = external_api::clean_returnvalue(mod_setask_external::revert_submissions_to_draft_returns(), $result);
    ...
    public function test_save_submission() {
        ...
        $params['course'] = $course->id;
        $params['setasksubmission_onlinetext_enabled'] = 1;
        $params['setasksubmission_file_enabled'] = 1;
        $params['setasksubmission_file_maxfiles'] = 5;
        $params['setasksubmission_file_maxsizebytes'] = 1024*1024;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('setask', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_setask_external::save_submission($instance->id, $submissionpluginparams);
        $result = external_api::clean_returnvalue(mod_setask_external::save_submission_returns(), $result);
    ...
    public function test_save_grade() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_setask');
        $params['course'] = $course->id;
        $params['setaskfeedback_file_enabled'] = 1;
        $params['setaskfeedback_comments_enabled'] = 1;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('setask', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_setask_external::save_grade($instance->id,
                                                  $student1->id,
                                                  50.0,
                                                  -1,
                                                  true,
        ...
        $result = mod_setask_external::get_grades(array($instance->id));
        $result = external_api::clean_returnvalue(mod_setask_external::get_grades_returns(), $result);

        $this->assertEquals($result['setaskments'][0]['grades'][0]['grade'], '50.0');
    }
    ...
    public function test_save_grades_with_advanced_grading() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_setask');
        $params['course'] = $course->id;
        $params['setaskfeedback_file_enabled'] = 0;
        $params['setaskfeedback_comments_enabled'] = 0;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('setask', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_setask_external::save_grades($instance->id, false, $grades);
        $this->assertNull($result);

        $student1grade = $DB->get_record('setask_grades',
                                         array('userid' => $student1->id, 'setaskment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals($student1grade->grade, '50.0');

        $student2grade = $DB->get_record('setask_grades',
                                         array('userid' => $student2->id, 'setaskment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
    ...
    public function test_save_grades_with_group_submission() {
        ...
        $student3grade = $DB->get_record('setask_grades',
                                         array('userid' => $student3->id, 'setaskment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals($student3grade->grade, '50.0');

        $student4grade = $DB->get_record('setask_grades',
                                         array('userid' => $student4->id, 'setaskment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
    ...
    public function test_copy_previous_attempt() {
        ...
        $params['course'] = $course->id;
        $params['setasksubmission_onlinetext_enabled'] = 1;
        $params['setasksubmission_file_enabled'] = 0;
        $params['setaskfeedback_file_enabled'] = 0;
        ...
        $result = mod_setask_external::copy_previous_attempt($instance->id);
        $result = external_api::clean_returnvalue(mod_setask_external::copy_previous_attempt_returns(), $result);
        // No warnings.
        $this->assertEquals(0, count($result));

        $this->setUser($teacher);
        $result = mod_setask_external::get_submissions(array($instance->id));
        $result = external_api::clean_returnvalue(mod_setask_external::get_submissions_returns(), $result);

        // Check we are now on the second attempt.
        $this->assertEquals($result['setaskments'][0]['submissions'][0]['attemptnumber'], 1);
        // Check the plugins data is not empty.
        $this->assertNotEmpty($result['setaskments'][0]['submissions'][0]['plugins']);
    ...
    public function test_set_user_flags() {
        ...
        $createduserflags = mod_setask_external::set_user_flags($setask->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $createduserflags = external_api::clean_returnvalue(mod_setask_external::set_user_flags_returns(), $createduserflags);

        $this->assertEquals($student->id, $createduserflags[0]['userid']);
        $createduserflag = $DB->get_record('setask_user_flags', array('id' => $createduserflags[0]['id']));

        // Confirm that all data was inserted correctly.
        $this->assertEquals($student->id,  $createduserflag->userid);
        $this->assertEquals($setask->id, $createduserflag->setaskment);
        ...
        $updateduserflags = mod_setask_external::set_user_flags($setask->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $updateduserflags = external_api::clean_returnvalue(mod_setask_external::set_user_flags_returns(), $updateduserflags);

        $this->assertEquals($student->id, $updateduserflags[0]['userid']);
        $updateduserflag = $DB->get_record('setask_user_flags', array('id' => $updateduserflags[0]['id']));

        // Confirm that all data was updated correctly.
        $this->assertEquals($student->id,  $updateduserflag->userid);
        $this->assertEquals($setask->id, $updateduserflag->setaskment);
