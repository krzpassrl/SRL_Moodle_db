 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_setask_grading_batch_operations_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        // Visible elements.
        $options = array();
        $options['lock'] = get_string('locksubmissions', 'setask');
        $options['unlock'] = get_string('unlocksubmissions', 'setask');
        if ($instance['submissiondrafts']) {
            $options['reverttodraft'] = get_string('reverttodraft', 'setask');
        }
        if ($instance['duedate'] && has_capability('mod/setask:grantextension', $instance['context'])) {
            $options['grantextension'] = get_string('grantextension', 'setask');
        }
        if ($instance['attemptreopenmethod'] == ASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL) {
            $options['addattempt'] = get_string('addattempt', 'setask');
        }
        ...
        if ($instance['markingworkflow']) {
            $options['setmarkingworkflowstate'] = get_string('setmarkingworkflowstate', 'setask');
        }
        if ($instance['markingallocation']) {
            $options['setmarkingallocation'] = get_string('setmarkingallocation', 'setask');
        }
        ...
        $objs = array();
        $objs[] =& $mform->createElement('select', 'operation', get_string('chooseoperation', 'setask'), $options);
        $objs[] =& $mform->createElement('submit', 'submit', get_string('go'));
        $batchdescription = get_string('batchoperationsdescription', 'setask');
