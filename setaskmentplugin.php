...
/**
 * This file contains the functions for setask_plugin abstract class
 *
 *
 * @package   mod_setask
...
/**
 * Abstract class for setask_plugin (submission/feedback).
 *
 * @package   mod_setask
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class setask_plugin {

    /** @var setask $setaskment the setaskment record that contains the global
     *              settings for this setask instance
     */
    protected $setaskment;
    /** @var string $type setaskment plugin type */
    private $type = '';
     ...
     *
     * @param setask $setaskment
     * @param string $type
     */
    public final function __construct(setask $setaskment, $type) {
     ...
     * form for display in the settings page for the setaskment.
    ...
    /**
     * The setaskment subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'setasksubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - setask_submission or setask_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this setaskment.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/setask/' . substr($this->get_subtype(), strlen('setask')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('setaskment'=>$this->setaskment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('setask_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('setask_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->setaskment = $this->setaskment->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('setask_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            if (!$this->setaskment->has_instance()) {
                return false;
            }
            $setaskment = $this->setaskment->get_instance();
            if ($setaskment) {
                $dbparams = array('setaskment'=>$setaskment->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('setask_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('setaskment'=>$this->setaskment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('setask_plugin_config', $dbparams);
     ...
     * and version.
     *
     * @param string $type The old setaskment subtype
     * @param int $version The old setaskment version
    ...
    /**
     * Upgrade the settings from the old setaskment to the new one
     *
     * @param context $oldcontext The context for the old setaskment module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldsetaskment, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_setask', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade setask_submission or setask_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_setask', $params);
        return false;
    }
    ...
    /**
     * The setaskment has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this setaskment plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade setask_submission or setask_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if (!($storedfile = $fs->get_file($this->setaskment->get_context()->id,
    ...
    /**
     * This allows a plugin to render a page in the context of the setaskment
     *
     * If the plugin creates a link to the setaskment view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=setaskfeedback|setasksubmission
     ...
     * right below the activity's "intro" section on the main setaskment page.
     *
     * @return string
     */
     ...
     * enabled/disabled per setaskment instance.
     *
     * @return bool
     */
