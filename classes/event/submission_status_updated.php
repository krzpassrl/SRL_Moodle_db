...
/**
 * The mod_setask submission status updated event.
 *
 * @package    mod_setask
...
/**
 * The mod_setask submission status updated event class.
 *
...
class submission_status_updated extends base {
    ...
    public static function create_from_submission(\setask $setask, \stdClass $submission) {
        $data = array(
            ...
            'relateduserid' => ($setask->get_instance()->teamsubmission) ? null : $submission->userid,
            'other' => array(
    ...
    public function get_description() {
        return "The user with id '$this->userid' has updated the status of the submission with id '$this->objectid' for " .
            "the setaskment with course module id '$this->contextinstanceid' to the status '{$this->other['newstatus']}'.";
    }
    ...
    public static function get_name() {
        return get_string('eventsubmissionstatusupdated', 'mod_setask');
    }
    ...
    protected function init() {
        ...
        $this->data['objecttable'] = 'setask_submission';
    }
    ...
    protected function get_legacy_logdata() {
        $submission = $this->get_record_snapshot('setask_submission', $this->objectid);
        $user = $this->get_record_snapshot('user', $submission->userid);
        $logmessage = get_string('reverttodraftforstudent', 'setask', array('id' => $user->id, 'fullname' => fullname($user)));
