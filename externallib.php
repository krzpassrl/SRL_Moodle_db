 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_setask_external extends external_api {

    /**
     * Generate a warning in a standard structure for a known failure.
     *
     * @param int $setaskmentid - The setaskment
     ...
     */
    private static function generate_warning($setaskmentid, $warningcode, $detail) {
        $warningmessages = array(
            'couldnotlock'=>'Could not lock the submission for this user.',
            'couldnotunlock'=>'Could not unlock the submission for this user.',
            'couldnotsubmitforgrading'=>'Could not submit setaskment for grading.',
    ...
    public static function get_grades_parameters() {
        return new external_function_parameters(
            array(
                'setaskmentids' => new external_multiple_structure(
                    new external_value(PARAM_INT, 'setaskment id'),
                    '1 or more setaskment ids',
                    VALUE_REQUIRED),
    ...
    /**
     * Returns grade information from setask_grades for the requested setaskment ids
     * @param int[] $setaskmentids
     * @param int $since only return records with timemodified >= since
     * @return array of grade records for each requested setaskment
     * @since  Moodle 2.4
     */
    public static function get_grades($setaskmentids, $since = 0) {
        global $DB;
        $params = self::validate_parameters(self::get_grades_parameters(),
                        array('setaskmentids' => $setaskmentids,
                              'since' => $since));

        $setaskments = array();
        $warnings = array();
        $requestedsetaskmentids = $params['setaskmentids'];

        // Check the user is allowed to get the grades for the setaskments requested.
        $placeholders = array();
        list($sqlsetaskmentids, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlsetaskmentids;
        ...
        foreach ($cms as $cm) {
            try {
                ...
                $requestedsetaskmentids = array_diff($requestedsetaskmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'setaskment';
        ...
        if (count ($requestedsetaskmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);
            ...
            $sql = "SELECT ag.id,
                           ...
                           ag.grade,
                           ag.attemptnumber
                      FROM {setask_grades} ag, {setask_submission} s
                     WHERE s.setaskment $inorequalsql
                       AND s.userid = ag.userid
                       AND s.latest = 1
                       AND s.attemptnumber = ag.attemptnumber
                       AND ag.timemodified  >= :since
                       AND ag.setaskment = s.setaskment
                  ORDER BY ag.setaskment, ag.id";

            $placeholders['since'] = $params['since'];
            $rs = $DB->get_recordset_sql($sql, $placeholders);
            $currentsetaskmentid = null;
            $setaskment = null;
            foreach ($rs as $rd) {
                $grade = array();
                $grade['id'] = $rd->id;
                ...
                if (is_null($currentsetaskmentid) || ($rd->setaskment != $currentsetaskmentid )) {
                    if (!is_null($setaskment)) {
                        $setaskments[] = $setaskment;
                    }
                    $setaskment = array();
                    $setaskment['setaskmentid'] = $rd->setaskment;
                    $setaskment['grades'] = array();
                    $requestedsetaskmentids = array_diff($requestedsetaskmentids, array($rd->setaskment));
                }
                $setaskment['grades'][] = $grade;

                $currentsetaskmentid = $rd->setaskment;
            }
            if (!is_null($setaskment)) {
                $setaskments[] = $setaskment;
            }
        ...
        foreach ($requestedsetaskmentids as $setaskmentid) {
            $warning = array();
            $warning['item'] = 'setaskment';
            $warning['itemid'] = $setaskmentid;
        ...
        $result = array();
        $result['setaskments'] = $setaskments;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Creates an setask_grades external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.4
     */
    private static function setask_grades() {
        return new external_single_structure(
            array (
                'setaskmentid'    => new external_value(PARAM_INT, 'setaskment id'),
    ...
    public static function get_grades_returns() {
        return new external_single_structure(
            array(
                'setaskments' => new external_multiple_structure(self::setask_grades(), 'list of setaskment grade information'),
                'warnings'      => new external_warnings('item is always \'setaskment\'',
                    'when errorcode is 3 then itemid is an setaskment id. When errorcode is 1, itemid is a course module id',
     ...
     * @since  Moodle 2.4
     */
    public static function get_setaskments_parameters() {
        return new external_function_parameters(
            array(
    ...
    /**
     * Returns an array of courses the user is enrolled in, and for each course all of the setaskments that the user can
     * view within that course.
     *
     * @param array $courseids An optional array of course ids. If provided only setaskments within the given course
     ...
     * @since  Moodle 2.4
     */
    public static function get_setaskments($courseids = array(), $capabilities = array()) {
        global $USER, $DB, $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");

        $params = self::validate_parameters(
            self::get_setaskments_parameters(),
        ...
        $extrafields='m.id as setaskmentid, ' .
                     'm.course, ' .
                     'm.nosubmissions, ' .
                     'm.submissiondrafts, ' .
                     'm.sendnotifications, '.
        ...
        $coursearray = array();
        foreach ($courses as $id => $course) {
            $setaskmentarray = array();
            // Get a list of setaskments for the course.
            if ($modules = get_coursemodules_in_course('setask', $courses[$id]->id, $extrafields)) {
                foreach ($modules as $module) {
                    ...
                    $configrecords = $DB->get_recordset('setask_plugin_config', array('setaskment' => $module->setaskmentid));
                    $configarray = array();
                    foreach ($configrecords as $configrecord) {
                        $configarray[] = array(
                            'id' => $configrecord->id,
                            'setaskment' => $configrecord->setaskment,
                    ...
                    if ($setask->show_intro()) {
                        ...
                        list($setaskment['intro'], $setaskment['introformat']) = external_format_text($module->intro,
                            $module->introformat, $context->id, 'mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA, 0);

                        $fs = get_file_storage();
                        if ($files = $fs->get_area_files($context->id, 'mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA,
                                                            0, 'timemodified', false)) {

                            $setaskment['introattachments'] = array();
                            foreach ($files as $file) {
                                ...
                                $setaskment['introattachments'][] = array(
                                    ...
                                    'fileurl'  => moodle_url::make_webservice_pluginfile_url(
                                        $context->id, 'mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA, 0, '/', $filename)->out(false)
                                );
                            }
                        }
                    }

                    $setaskmentarray[] = $setaskment;
                }
            }
            $coursearray[]= array(
                'id' => $courses[$id]->id,
    ...
    /**
     * Creates an setaskment external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_setaskments_setaskment_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'setaskment id'),
                'cmid' => new external_value(PARAM_INT, 'course module id'),
                'course' => new external_value(PARAM_INT, 'course id'),
                'name' => new external_value(PARAM_TEXT, 'setaskment name'),
                ...
                'duedate' => new external_value(PARAM_INT, 'setaskment due date'),
                ...
                'timemodified' => new external_value(PARAM_INT, 'last time setaskment was modified'),
                ...
                'revealidentities' => new external_value(PARAM_INT, 'show identities for a blind marking setaskment'),
                ...
                'configs' => new external_multiple_structure(self::get_setaskments_config_structure(), 'configuration settings'),
                'intro' => new external_value(PARAM_RAW,
                    'setaskment intro, not allways returned because it deppends on the activity configuration', VALUE_OPTIONAL),
            ...
            ), 'setaskment information object');
    }

    /**
     * Creates an setask_plugin_config external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_setaskments_config_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'setask_plugin_config id'),
                'setaskment' => new external_value(PARAM_INT, 'setaskment id'),
                'plugin' => new external_value(PARAM_TEXT, 'plugin'),
                'subtype' => new external_value(PARAM_TEXT, 'subtype'),
                'name' => new external_value(PARAM_TEXT, 'name'),
                'value' => new external_value(PARAM_TEXT, 'value')
            ), 'setaskment configuration object'
        );
    }
     ...
     * @since Moodle 2.4
     */
    private static function get_setaskments_course_structure() {
        return new external_single_structure(
            array(
                ...
                'setaskments' => new external_multiple_structure(self::get_setaskments_setaskment_structure(), 'setaskment info')
              ), 'course information object'
        );
    }

    /**
     * Describes the return value for get_setaskments
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    public static function get_setaskments_returns() {
        return new external_single_structure(
            array(
                'courses' => new external_multiple_structure(self::get_setaskments_course_structure(), 'list of courses'),
    ...
    public static function get_submissions($setaskmentids, $status = '', $since = 0, $before = 0) {
        ...
        foreach ($setasks as $setask) {
            ...
            $placeholders = array('setaskid1' => $setask->get_instance()->id,
                                  'setaskid2' => $setask->get_instance()->id);
            ...
            $sql = "SELECT mas.id, mas.setaskment,mas.userid,".
                   "mas.timecreated,mas.timemodified,mas.status,mas.groupid,mas.attemptnumber ".
                   "FROM {setask_submission} mas ".
                   "JOIN ( " . $submissionmaxattempt . " ) smx ON mas.userid = smx.userid ".
                   "WHERE mas.setaskment = :setaskid2 AND mas.attemptnumber = smx.maxattempt";
                    ...
                    foreach ($submissionplugins as $submissionplugin) {
                        ...
                        foreach ($fileareas as $filearea => $name) {
                            ...
                            foreach ($files as $file) {
                                ...
                                $fileurl = file_encode_url($CFG->wwwroot . '/webservice/pluginfile.php', '/' . $setask->get_context()->id .
            ...
            $setaskments[] = array(
                'setaskmentid' => $setask->get_instance()->id,
    ...
    /**
     * Creates an setask_submissions external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    ...
    public static function get_submissions_returns() {
        return new external_single_structure(
            array(
                'setaskments' => new external_multiple_structure(self::get_submissions_structure(), 'setaskment submissions'),
     ...
     *
     * @param int $setaskmentid the setaskment for which the userflags are created or updated
     ...
     * @since Moodle 2.6
     */
    public static function set_user_flags($setaskmentid, $userflags = array()) {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/mod/setask/locallib.php");

        $params = self::validate_parameters(self::set_user_flags_parameters(),
                                            array('setaskmentid' => $setaskmentid,
                                                  'userflags' => $userflags));

        // Load setaskment if it exists and if the user has the capability.
        $cm = get_coursemodule_from_instance('setask', $params['setaskmentid'], 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        self::validate_context($context);
        require_capability('mod/setask:grade', $context);
        $setask = new setask($context, null, null);
        ...
        foreach ($params['userflags'] as $userflag) {
            ...
            $record = $setask->get_user_flags($userflag['userid'], false);
            if ($record) {
                ...
                if ($setask->update_user_flags($record)) {
                    $result['id'] = $record->id;
                ...
                $record = $setask->get_user_flags($userflag['userid'], true);
    ...
    /**
     * Returns user flag information from setask_user_flags for the requested setaskment ids
     * @param int[] $setaskmentids
     * @return array of user flag records for each requested setaskment
     * @since  Moodle 2.6
     */
    public static function get_user_flags($setaskmentids) {
        global $DB;
        ...
        // Check the user is allowed to get the user flags for the setaskments requested.
        $placeholders = array();
        list($sqlsetaskmentids, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlsetaskmentids;
        ...
        // Create the query and populate an array of setask_user_flags records from the recordset results.
        if (count ($requestedsetaskmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT auf.id,auf.setaskment,auf.userid,auf.locked,auf.mailed,".
                   "auf.extensionduedate,auf.workflowstate,auf.allocatedmarker ".
                   "FROM {setask_user_flags} auf ".
                   "WHERE auf.setaskment ".$inorequalsql.
                   " ORDER BY auf.setaskment, auf.id";
            ...
            foreach ($rs as $rd) {
                ...
                if (is_null($currentsetaskmentid) || ($rd->setaskment != $currentsetaskmentid )) {
                    ...
                    $setaskment = array();
                    $setaskment['setaskmentid'] = $rd->setaskment;
                    $setaskment['userflags'] = array();
                    $requestedsetaskmentids = array_diff($requestedsetaskmentids, array($rd->setaskment));
                }
                $setaskment['userflags'][] = $userflag;
    ...
    /**
     * Creates an setask_user_flags external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function setask_user_flags() {
        return new external_single_structure(
            array (
                'setaskmentid'    => new external_value(PARAM_INT, 'setaskment id'),
    ...
    public static function get_user_flags_returns() {
        return new external_single_structure(
            array(
                'setaskments' => new external_multiple_structure(self::setask_user_flags(), 'list of setask user flag information'),
                'warnings'      => new external_warnings('item is always \'setaskment\'',
                    'when errorcode is 3 then itemid is an setaskment id. When errorcode is 1, itemid is a course module id',
    ...
    /**
     * Returns user mapping information from setask_user_mapping for the requested setaskment ids
     * @param int[] $setaskmentids
     * @return array of user mapping records for each requested setaskment
     * @since  Moodle 2.6
     */
    public static function get_user_mappings($setaskmentids) {
        global $DB;
        ...
        // Check the user is allowed to get the mappings for the setaskments requested.
        $placeholders = array();
        list($sqlsetaskmentids, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlsetaskmentids;
        ...
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/setask:revealidentities', $context);
            } catch (Exception $e) {
                $requestedsetaskmentids = array_diff($requestedsetaskmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'setaskment';
        ...
        // Create the query and populate an array of setask_user_mapping records from the recordset results.
        if (count ($requestedsetaskmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedsetaskmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT aum.id,aum.setaskment,aum.userid ".
                   "FROM {setask_user_mapping} aum ".
                   "WHERE aum.setaskment ".$inorequalsql.
                   " ORDER BY aum.setaskment, aum.id";
            ...
            foreach ($rs as $rd) {
                ...
                if (is_null($currentsetaskmentid) || ($rd->setaskment != $currentsetaskmentid )) {
                    ...
                    $setaskment = array();
                    $setaskment['setaskmentid'] = $rd->setaskment;
                    $setaskment['mappings'] = array();
                    $requestedsetaskmentids = array_diff($requestedsetaskmentids, array($rd->setaskment));
                }
                $setaskment['mappings'][] = $mapping;
    ...
    /**
     * Creates an setask_user_mappings external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function setask_user_mappings() {
        return new external_single_structure(
            array (
                'setaskmentid'    => new external_value(PARAM_INT, 'setaskment id'),
    ...
    public static function get_user_mappings_returns() {
        return new external_single_structure(
            array(
                'setaskments' => new external_multiple_structure(self::setask_user_mappings(), 'list of setask user mapping data'),
                'warnings'      => new external_warnings('item is always \'setaskment\'',
                    'when errorcode is 3 then itemid is an setaskment id. When errorcode is 1, itemid is a course module id',
    ...
    public static function lock_submissions_parameters() {
        return new external_function_parameters(
            array(
                'setaskmentid' => new external_value(PARAM_INT, 'The setaskment id to operate on'),
    ...
    /**
     * Locks (prevent updates to) submissions in this setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     * @param array $userids Array of user ids to lock
     * @return array of warnings for each submission that could not be locked.
     * @since Moodle 2.6
     */
    public static function lock_submissions($setaskmentid, $userids) {
        global $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $setaskment = new setask($context, $cm, null);

        $warnings = array();
        foreach ($params['userids'] as $userid) {
            if (!$setaskment->lock_submission($userid)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['setaskmentid'];
    ...
    /**
     * Reverts a list of user submissions to draft for a single setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     ...
     * @since Moodle 2.6
     */
    public static function revert_submissions_to_draft($setaskmentid, $userids) {
        global $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $warnings = array();
        foreach ($params['userids'] as $userid) {
            if (!$setaskment->revert_to_draft($userid)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['setaskmentid'];
                $warnings[] = self::generate_warning($params['setaskmentid'],
                                                     'couldnotrevert',
                                                     $detail);
     ...
     * @since Moodle 2.6
     */
    public static function unlock_submissions($setaskmentid, $userids) {
        global $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $warnings = array();
        foreach ($params['userids'] as $userid) {
            if (!$setaskment->unlock_submission($userid)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['setaskmentid'];
                $warnings[] = self::generate_warning($params['setaskmentid'],
                                                     'couldnotunlock',
                                                     $detail);
    ...
    public static function submit_for_grading_parameters() {
        return new external_function_parameters(
            array(
                'setaskmentid' => new external_value(PARAM_INT, 'The setaskment id to operate on'),
                'acceptsubmissionstatement' => new external_value(PARAM_BOOL, 'Accept the setaskment submission statement')
            )
        );
    }

    /**
     * Submit the logged in users setaskment for grading.
     *
     * @param int $setaskmentid The id of the setaskment
     * @return array of warnings to indicate any errors.
     * @since Moodle 2.6
     */
    public static function submit_for_grading($setaskmentid, $acceptsubmissionstatement) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        if (!$setaskment->submit_for_grading($data, $notices)) {
            $detail = 'User id: ' . $USER->id . ', Assignment id: ' . $params['setaskmentid'] . ' Notices:' . implode(', ', $notices);
            $warnings[] = self::generate_warning($params['setaskmentid'],
    ...
    /**
     * Grant extension dates to students for an setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     ...
     * @since Moodle 2.6
     */
    public static function save_user_extensions($setaskmentid, $userids, $dates) {
        global $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        foreach ($params['userids'] as $idx => $userid) {
            ...
            if (!$setaskment->save_user_extension($userid, $duedate)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['setaskmentid'] . ', Extension date: ' . $duedate;
                $warnings[] = self::generate_warning($params['setaskmentid'],
    ...
    /**
     * Reveal the identities of anonymous students to markers for a single setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     * @return array of warnings to indicate any errors.
     * @since Moodle 2.6
     */
    public static function reveal_identities($setaskmentid) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $warnings = array();
        if (!$setaskment->reveal_identities()) {
            $detail = 'User id: ' . $USER->id . ', Assignment id: ' . $params['setaskmentid'];
            $warnings[] = self::generate_warning($params['setaskmentid'],
    ...
    public static function save_submission_parameters() {
        global $CFG;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        $instance = new setask(null, null, null);
    ...
    /**
     * Save a student submission for a single setaskment
     *
     * @param int $setaskmentid The id of the setaskment
     ...
     * @since Moodle 2.6
     */
    public static function save_submission($setaskmentid, $plugindata) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $setaskment->save_submission($submissiondata, $notices);
        ...
        return new external_function_parameters(
            array(
                ...
                'applytoall' => new external_value(PARAM_BOOL, 'If true, this grade will be applied ' .
                                                               'to all members ' .
                                                               'of the group (for group setaskments).'),
    ...
    /**
     * Save a student grade for a single setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     * @param int $userid The id of the user
     * @param float $grade The grade (ignored if the setaskment uses advanced grading)
     ...
     * @since Moodle 2.6
     */
    public static function save_grade($setaskmentid,
                                      $userid,
                                      $grade,
                                      $attemptnumber,
                                      $addattempt,
        ...
        $setaskment->save_grade($params['userid'], $gradedata);
    ...
    /**
     * Save multiple student grades for a single setaskment.
     *
     * @param int $setaskmentid The id of the setaskment
     * @param boolean $applytoall If set to true and this is a team setaskment,
     ...
     *                  grade - The grade (ignored if the setaskment uses advanced grading)
     ...
     * a team setaskment that has $applytoall set to true
     * @return null
     * @since Moodle 2.7
     */
    public static function save_grades($setaskmentid, $applytoall = false, $grades) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        if ($setaskment->get_instance()->teamsubmission && $params['applytoall']) {
            // Check that only 1 user per submission group is provided.
            $groupids = array();
            foreach ($params['grades'] as $gradeinfo) {
                $group = $setaskment->get_submission_group($gradeinfo['userid']);
        ...
        foreach ($params['grades'] as $gradeinfo) {
            ...
            $setaskment->save_grade($gradeinfo['userid'], $gradedata);
        }
     ...
     * @since Moodle 2.6
     */
    public static function copy_previous_attempt($setaskmentid) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/setask/locallib.php");
        ...
        $cm = get_coursemodule_from_instance('setask', $setaskmentid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        self::validate_context($context);
        ...
        $setaskment->copy_previous_attempt($submissiondata, $notices);

        $warnings = array();
        foreach ($notices as $notice) {
            $warnings[] = self::generate_warning($setaskmentid,
