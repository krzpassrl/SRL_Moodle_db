...
/**
 * This file contains the classes for the admin settings of the setask module.
 *
 * @package   mod_setask
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_admin_page_manage_setask_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/setask/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'setask'),
                            $url);
    }
...
class setask_plugin_manager {
    ...
    /**
     * Constructor for this setaskment plugin manager
     * @param string $subtype - either setasksubmission or setaskfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/setask/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'setask'),
                get_string('version'), get_string('hideshow', 'setask'),
        ...
        $shortsubtype = substr($this->subtype, strlen('setask'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/setask/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'setask'));
    }
