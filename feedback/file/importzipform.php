 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setaskfeedback_file_import_zip_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'uploadzip', get_string('confirmuploadzip', 'setaskfeedback_file'));
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($importer->is_valid_filename_for_import($setaskment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($importer->is_file_modified($setaskment, $user, $plugin, $filename, $unzippedfile)) {
                    // Get a string we can show to identify this user.
                    $userdesc = fullname($user);
                    if ($setaskment->is_blind_marking()) {
                        $userdesc = get_string('hiddenuser', 'setask') .
                                    $setaskment->get_uniqueid_for_user($user->id);
                    }
                    $grade = $setaskment->get_user_grade($user->id, false);
                    ...
                    if (!$grade || !$exists) {
                        $updates[] = get_string('feedbackfileadded', 'setaskfeedback_file',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    } else {
                        $updates[] = get_string('feedbackfileupdated', 'setaskfeedback_file',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    }
                }
            }
        ...
        if (count($updates)) {
            ...
            $mform->addElement('html', get_string('nochanges', 'setaskfeedback_file'));
        }
