 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setaskfeedback_file_zip_importer {
     ...
     * @param setask_plugin $plugin - Set to the plugin that exported the file
     ...
     */
    public function is_valid_filename_for_import($setaskment, $fileinfo, $participants, & $user, & $plugin, & $filename) {
        ...
        $filename = $info[4];
        $plugin = $setaskment->get_plugin_by_type($info[2], $info[3]);
    ...
    public function is_file_modified($setaskment, $user, $plugin, $filename, $fileinfo) {
        ...
        } else if ($plugin->get_subtype() == 'setaskfeedback') {
     ...
     *
     * @param int $contextid - The context id of this setaskment instance
     ...
     * @param int $contextid The context for this setaskment
    ...
    public function import_zip_files($setaskment, $fileplugin) {
        ...
        $importsummary = new setaskfeedback_file_import_summary($setaskment->get_course_module()->id,
        ...
        $o .= $setaskrenderer->render(new setask_header($setaskment->get_instance(),
                                                        $setaskment->get_context(),
                                                        false,
                                                        $setaskment->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'setaskfeedback_file')));
