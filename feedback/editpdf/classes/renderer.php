 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setaskfeedback_editpdf_renderer extends plugin_renderer_base {
    ...
    private function render_toolbar_button($icon, $tool, $accesskey = null, $disabled=false) {
        ...
        $iconalt = get_string('toolbarbutton', 'setaskfeedback_editpdf', $alttext);

        $iconhtml = $this->pix_icon($icon, $iconalt, 'setaskfeedback_editpdf');
     ...
     *
     * @param setaskfeedback_editpdf_widget $widget - Renderable widget containing setaskment, user and attempt number.
     * @return string
     */
    public function render_setaskfeedback_editpdf_widget(setaskfeedback_editpdf_widget $widget) {
        global $CFG;

        $html = '';

        $html .= html_writer::div(get_string('jsrequired', 'setaskfeedback_editpdf'), 'hiddenifjs');
        $linkid = html_writer::random_id();
        if ($widget->readonly) {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('viewfeedbackonline', 'setaskfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        } else {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('launcheditor', 'setaskfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        }
        $links = $launcheditorlink;

        $links .= html_writer::tag('div',
                                   get_string('unsavedchanges', 'setaskfeedback_editpdf'),
                                   array('class'=>'setaskfeedback_editpdf_unsavedchanges warning'));

        $html .= html_writer::div($links, 'visibleifjs');
        $header = get_string('pluginname', 'setaskfeedback_editpdf');
        $body = '';
        ...
        $iconalt = get_string('navigateprevious', 'setaskfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_prev, $iconalt, 'setaskfeedback_editpdf');
        ...
        $navigation1 .= html_writer::tag('select', null, array('disabled'=>'true',
            'aria-label' => get_string('gotopage', 'setaskfeedback_editpdf'), 'class'=>'navigate-page-select',
            'accesskey' => $this->get_shortcut('navigate-page-select')));
        $iconalt = get_string('navigatenext', 'setaskfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_next, $iconalt, 'setaskfeedback_editpdf');
        $navigation1 .= html_writer::tag('button', $iconhtml, array('disabled'=>'true',
        ...
        $progressbar = html_writer::div($progressbar, 'progress progress-info progress-striped active',
            array('title' => get_string('loadingeditor', 'setaskfeedback_editpdf'),
                  'role'=> 'progressbar', 'aria-valuenow' => 0, 'aria-valuemin' => 0,
                  'aria-valuemax' => 100));
        $progressbarlabel = html_writer::div(get_string('generatingpdf', 'setaskfeedback_editpdf'),
            'progressbarlabel');
        ...
        $editorparams = array(array('header'=>$header,
                                    ...
                                    'footer'=>$footer,
                                    'linkid'=>$linkid,
                                    'setaskmentid'=>$widget->setaskment,
        ...
        $this->page->requires->yui_module('moodle-setaskfeedback_editpdf-editor',
                                          'M.setaskfeedback_editpdf.editor.init',
                                          $editorparams);
