...
/**
 * This file contains the ingest manager for the setaskfeedback_editpdf plugin
 *
...
namespace setaskfeedback_editpdf;
...
class document_services {
    ...
    /**
     * This function will take an int or an setaskment instance and
     * return an setaskment instance. It is just for convenience.
     * @param int|\setask $setaskment
     * @return setask
     */
    private static function get_setaskment_from_param($setaskment) {
        global $CFG;

        require_once($CFG->dirroot . '/mod/setask/locallib.php');

        if (!is_object($setaskment)) {
            $cm = \get_coursemodule_from_instance('setask', $setaskment, 0, false, MUST_EXIST);
     ...
     */
    private static function hash($setaskment, $userid, $attemptnumber) {
        if (is_object($setaskment)) {
            $setaskmentid = $setaskment->get_instance()->id;
        } else {
            $setaskmentid = $setaskment;
        }
        return sha1($setaskmentid . '_' . $userid . '_' . $attemptnumber);
    }
     ...
     */
    public static function list_compatible_submission_files_for_attempt($setaskment, $userid, $attemptnumber) {
        global $USER, $DB;

        $setaskment = self::get_setaskment_from_param($setaskment);

        // Capability checks.
        if (!$setaskment->can_view_submission($userid)) {
        ...
        if ($setaskment->get_instance()->teamsubmission) {
            $submission = $setaskment->get_group_submission($userid, 0, false);
        } else {
            $submission = $setaskment->get_user_submission($userid, false);
        }
     ...
     * @return stored_file
     */
    public static function get_combined_pdf_for_attempt($setaskment, $userid, $attemptnumber) {
        ...
        $grade = $setaskment->get_user_grade($userid, true, $attemptnumber);
        if ($setaskment->get_instance()->teamsubmission) {
            $submission = $setaskment->get_group_submission($userid, 0, false);
        } else {
            $submission = $setaskment->get_user_submission($userid, false);
        ...
        if (!$combinedpdf ||
            ...
            return self::generate_combined_pdf_for_attempt($setaskment, $userid, $attemptnumber);
        }
        return $combinedpdf;
    }
     ...
     * @return stored_file
     */
    public static function generate_combined_pdf_for_attempt($setaskment, $userid, $attemptnumber) {
        global $CFG;
        ...
        $files = self::list_compatible_submission_files_for_attempt($setaskment, $userid, $attemptnumber);
        ...
        if ($files) {
            ...
            $tmpdir = \make_temp_directory('setaskfeedback_editpdf/combined/' . self::hash($setaskment, $userid, $attemptnumber));
        ...
        $record->contextid = $setaskment->get_context()->id;
        $record->component = 'setaskfeedback_editpdf';
     ...
     */
    public static function page_number_for_attempt($setaskment, $userid, $attemptnumber, $readonly = false) {
        global $CFG;
        ...
        if ($readonly) {
            $grade = $setaskment->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($setaskment->get_context()->id, 'setaskfeedback_editpdf',
        ...
        $file = self::get_combined_pdf_for_attempt($setaskment, $userid, $attemptnumber);
        if (!$file) {
            \print_error('Could not generate combined pdf.');
        }

        // Store the combined pdf file somewhere to be opened by tcpdf.
        $tmpdir = \make_temp_directory('setaskfeedback_editpdf/pagetotal/'
            . self::hash($setaskment, $userid, $attemptnumber));
     ...
     * @return array(stored_file)
     */
    public static function generate_page_images_for_attempt($setaskment, $userid, $attemptnumber) {
        global $CFG;
        ...
        $tmpdir = \make_temp_directory('setaskfeedback_editpdf/pageimages/' . self::hash($setaskment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
     ...
     * @return array(stored_file)
     */
    public static function get_page_images_for_attempt($setaskment, $userid, $attemptnumber, $readonly = false) {
        ...
        if ($readonly) {
            ...
            if ($fs->is_area_empty($contextid, $component, $filearea, $itemid)) {
                ...
                self::generate_page_images_for_attempt($setaskment, $userid, $attemptnumber);
                self::copy_pages_to_readonly_area($setaskment, $grade);
            }
        }
        ...
        if (empty($pages)) {
            ...
            $pages = self::generate_page_images_for_attempt($setaskment, $userid, $attemptnumber);
        }
     ...
     * @return string
     */
    protected static function get_downloadable_feedback_filename($setaskment, $userid, $attemptnumber) {
        global $DB;

        $setaskment = self::get_setaskment_from_param($setaskment);

        $groupmode = groups_get_activity_groupmode($setaskment->get_course_module());
        $groupname = '';
        if ($groupmode) {
            $groupid = groups_get_activity_group($setaskment->get_course_module(), true);
        ...
        if ($setaskment->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'setask');
            $prefix = str_replace('_', ' ', $prefix);
            $prefix = clean_filename($prefix . '_' . $setaskment->get_uniqueid_for_user($userid) . '_');
        } else {
     ...
     * @return stored_file
     */
    public static function generate_feedback_document($setaskment, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('setaskfeedback_editpdf/final/' . self::hash($setaskment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
        ...
        $fs = \get_file_storage();
        $stamptmpdir = \make_temp_directory('setaskfeedback_editpdf/stamps/' . self::hash($setaskment, $userid, $attemptnumber));
        $grade = $setaskment->get_user_grade($userid, true, $attemptnumber);
        // Copy any new stamps to this instance.
        if ($files = $fs->get_area_files($setaskment->get_context()->id,
                                         'setaskfeedback_editpdf',
                                         'stamps',
                                         $grade->id,
                                         "filename",
                                         false)) {
        ...
        $filename = self::get_downloadable_feedback_filename($setaskment, $userid, $attemptnumber);
     ...
     *
     * @param int|\setask $setaskment The setaskment.
     * @param \stdClass $grade The grade record.
     * @return void
     */
    public static function copy_pages_to_readonly_area($setaskment, $grade) {
        $fs = get_file_storage();
        $setaskment = self::get_setaskment_from_param($setaskment);
        $contextid = $setaskment->get_context()->id;
        $component = 'setaskfeedback_editpdf';
     ...
     * @return stored_file
     */
    public static function get_feedback_document($setaskment, $userid, $attemptnumber) {
     ...
     * @return bool
     */
    public static function delete_feedback_document($setaskment, $userid, $attemptnumber) {
