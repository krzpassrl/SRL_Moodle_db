 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_feedback_editpdf extends setask_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'setaskfeedback_editpdf');
    }
    ...
    public function get_widget($userid, $grade, $readonly) {
        ...
        $feedbackfile = document_services::get_feedback_document($this->setaskment->get_instance()->id,
                                                                 $userid,
                                                                 $attempt);
        ...
        if ($files = $fs->get_area_files($syscontext->id,
            ...
            foreach ($files as $file) {
                ...
                if ($filename !== '.') {
                    ...
                    $existingfile = $fs->get_file($this->setaskment->get_context()->id,
                                                  'setaskfeedback_editpdf',
                                                  'stamps',
                                                  $grade->id,
                                                  '/',
        ...
        if ($files = $fs->get_area_files($this->setaskment->get_context()->id,
                                         'setaskfeedback_editpdf',
                                         'stamps',
                                         $grade->id,
                                         "filename",
            ...
            foreach ($files as $file) {
                $filename = $file->get_filename();
                if ($filename !== '.') {
                    $url = moodle_url::make_pluginfile_url($this->setaskment->get_context()->id,
                                                   'setaskfeedback_editpdf',
                                                   'stamps',
                                                   $grade->id,
                                                   '/',
        ...
        $pagetotal = document_services::page_number_for_attempt($this->setaskment->get_instance()->id,
                $userid,
                $attempt,
                $readonly);

        $widget = new setaskfeedback_editpdf_widget($this->setaskment->get_instance()->id,
                                                    $userid,
                                                    $attempt,
                                                    $url,
                                                    $filename,
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        if (!empty($data->editpdf_source_userid)) {
            ...
            if ($sourceuserid != $grade->userid) {
                page_editor::copy_drafts_from_to($this->setaskment, $grade, $sourceuserid);
            }
        }
        if (page_editor::has_annotations_or_comments($grade->id, true)) {
            document_services::generate_feedback_document($this->setaskment, $grade->userid, $grade->attemptnumber);
        }
    ...
    public function view(stdClass $grade) {
        ...
        if (page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->setaskment->render_area_files('setaskfeedback_editpdf',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('setaskfeedback_editpdf_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('setaskfeedback_editpdf_annot', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('setask_grades', array('setaskment'=>$this->setaskment->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('setaskfeedback_editpdf_annot', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('setaskfeedback_editpdf_cmnt', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $testpath = setaskfeedback_editpdf\pdf::test_gs_path(false);
            $this->enabledcache = ($testpath->status == setaskfeedback_editpdf\pdf::GSPATH_OK);
        }
