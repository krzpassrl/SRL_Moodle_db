...
$setaskmentid = required_param('setaskmentid', PARAM_INT);
...
require_login($setaskment->get_course(), false, $cm);
...
if ($action == 'loadallpages') {
    ...
    if (!has_capability('mod/setask:grade', $context)) {
        ...
        require_capability('mod/setask:submit', $context);
    }
    ...
    $pages = document_services::get_page_images_for_attempt($setaskment,
                                                            $userid,
                                                            $attemptnumber,
                                                            $readonly);
    ...
    if ($added != count($page->comments)) {
        array_push($response->errors, get_string('couldnotsavepage', 'setaskfeedback_editpdf', $index+1));
    }
    ...
    $response = new stdClass();
    $grade = $setaskment->get_user_grade($userid, true);
    $file = document_services::generate_feedback_document($setaskment, $userid, $attemptnumber);

    $response->url = '';
    if ($file) {
        $url = moodle_url::make_pluginfile_url($setaskment->get_context()->id,
                                               'setaskfeedback_editpdf',
