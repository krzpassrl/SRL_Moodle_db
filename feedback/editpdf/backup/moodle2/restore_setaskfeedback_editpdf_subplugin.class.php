 ...
 * to restore one setask_feedback subplugin.
 *
 * @package   setaskfeedback_editpdf
 * @copyright 2013 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_setaskfeedback_editpdf_subplugin extends restore_subplugin {

    /**
     * Returns the paths to be handled by the subplugin at setaskment level
     * @return array
     */
     ...
     * @param mixed $data
     */
    public function process_setaskfeedback_editpdf_files($data) {
        $data = (object)$data;

        // In this case the id is the old gradeid which will be mapped.
        $this->add_related_files('setaskfeedback_editpdf',
            \setaskfeedback_editpdf\document_services::FINAL_PDF_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('setaskfeedback_editpdf',
            \setaskfeedback_editpdf\document_services::PAGE_IMAGE_READONLY_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('setaskfeedback_editpdf', 'stamps', 'grade', null, $data->gradeid);
    }
     ...
     * @param mixed $data
     */
    public function process_setaskfeedback_editpdf_annotation($data) {
        global $DB;

        $data = (object)$data;
        $oldgradeid = $data->gradeid;
        // The mapping is set in the restore for the core setask activity
        // when a grade node is processed.
        $data->gradeid = $this->get_mappingid('grade', $data->gradeid);

        $DB->insert_record('setaskfeedback_editpdf_annot', $data);
     ...
     * @param mixed $data
     */
    public function process_setaskfeedback_editpdf_comment($data) {
        global $DB;
        ...
        $DB->insert_record('setaskfeedback_editpdf_cmnt', $data);
