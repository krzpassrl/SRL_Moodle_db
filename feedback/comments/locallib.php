 ...
 *
 * @package   setaskfeedback_comments
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_feedback_comments extends setask_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'setaskfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('setaskfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'setaskfeedback_comments');
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'setaskfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('setaskfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->setaskment = $this->setaskment->get_instance()->id;
                return $DB->insert_record('setaskfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->setaskfeedback_comments_commentinline));
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $mform->addElement('selectyesno',
                           'setaskfeedback_comments_commentinline',
                           get_string('commentinline', 'setaskfeedback_comments'));
        $mform->addHelpButton('setaskfeedback_comments_commentinline', 'commentinline', 'setaskfeedback_comments');
        $mform->setDefault('setaskfeedback_comments_commentinline', $default);
        // Disable comment online if comment feedback plugin is disabled.
        $mform->disabledIf('setaskfeedback_comments_commentinline', 'setaskfeedback_comments_enabled', 'notchecked');
   }
    ...
    protected function convert_submission_text_to_feedback($submission, $data) {
        ...
        $data->setaskfeedbackcomments_editor['text'] = $text;
        $data->setaskfeedbackcomments_editor['format'] = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $submission = $this->setaskment->get_user_submission($userid, false);
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->setaskfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
            $data->setaskfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
        } else {
        ...
        $mform->addElement('editor', 'setaskfeedbackcomments_editor', $this->get_name(), null, null);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->setaskfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->setaskfeedbackcomments_editor['format'];
            return $DB->update_record('setaskfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->setaskfeedbackcomments_editor['text'];
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldsetaskment, & $log) {
        if ($oldsetaskment->setaskmenttype == 'online') {
            $this->set_config('commentinline', $oldsetaskment->var1);
            return true;
        }
        return true;
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->setaskment = $this->setaskment->get_instance()->id;
        if (!$DB->insert_record('setaskfeedback_comments', $feedbackcomments) > 0) {
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the setaskment
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('setaskfeedback_comments',
    ...
    public function get_external_parameters() {
        ...
        return array('setaskfeedbackcomments_editor' => $editorstructure);
    }
