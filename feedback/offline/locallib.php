 ...
 *
 *
 * @package   setaskfeedback_offline
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_feedback_offline extends setask_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'setaskfeedback_offline');
    }
    ...
    /**
     * Loop through uploaded grades and update the grades for this setaskment
     *
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified) {
        ...
        $gradeimporter = new setaskfeedback_offline_grade_importer($importid, $this->setaskment);
        ...
        if (!$gradeimporter->init()) {
            $thisurl = new moodle_url('/mod/setask/view.php', array('action'=>'viewpluginpage',
                                                                     'pluginsubtype'=>'setaskfeedback',
                                                                     'plugin'=>'offline',
                                                                     'pluginaction'=>'uploadgrades',
                                                                     'id'=>$setaskment->get_course_module()->id));
            print_error('invalidgradeimport', 'setaskfeedback_offline', $thisurl);
            return;
        }
        // Does this setaskment use a scale?
        $scaleoptions = null;
        if ($this->setaskment->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id'=>-($this->setaskment->get_instance()->grade)))) {
                $scaleoptions = make_menu_from_list($scale->scale);
            }
        }
        // We may need to upgrade the gradebook comments after this update.
        $adminconfig = $this->setaskment->get_admin_config();
        ...
        while ($record = $gradeimporter->next()) {
            ...
            $usergrade = $this->setaskment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $record->grade) {
                ...
                $skip = true;
            } else if ($this->setaskment->grading_disabled($record->user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($this->setaskment->get_instance()->grade > -1) &&
                      (($record->grade < 0) || ($record->grade > $this->setaskment->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }

            if (!$skip) {
                $grade = $this->setaskment->get_user_grade($record->user->id, true);

                $grade->grade = $record->grade;
                $grade->grader = $USER->id;
                if ($this->setaskment->update_grade($grade)) {
                ...
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        ...
                        if (($plugin->get_subtype() . '_' . $plugin->get_type()) == $gradebookplugin) {
                            ...
                            $this->setaskment->update_grade($grade);
                        }
                    }
                }
            }
        ...
        $o .= $renderer->render(new setask_header($this->setaskment->get_instance(),
                                                  $this->setaskment->get_context(),
                                                  false,
                                                  $this->setaskment->get_course_module()->id,
                                                  get_string('importgrades', 'setaskfeedback_offline')));
        $o .= $renderer->box(get_string('updatedgrades', 'setaskfeedback_offline', $updatecount));
    ...
    public function upload_grades() {
        ...
        require_capability('mod/setask:grade', $this->setaskment->get_context());
        require_once($CFG->dirroot . '/mod/setask/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/setask/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/setask/feedback/offline/importgradeslib.php');
        require_once($CFG->libdir . '/csvlib.class.php');

        $mform = new setaskfeedback_offline_upload_grades_form(null,
        ...
        if ($mform->is_cancelled()) {
            ...
            $importid = csv_import_reader::get_new_iid('setaskfeedback_offline');
            $gradeimporter = new setaskfeedback_offline_grade_importer($importid, $this->setaskment);
            ...
            $mform = new setaskfeedback_offline_import_grades_form(null, array('setaskment'=>$this->setaskment,
                                                                       'csvdata'=>$csvdata,
            ...
            $o .= $renderer->render(new setask_header($this->setaskment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->setaskment->get_course_module()->id,
                                                            get_string('confirmimport', 'setaskfeedback_offline')));
            $o .= $renderer->render(new setask_form('confirmimport', $mform));
            $o .= $renderer->render_footer();
        } else if ($confirm) {
            ...
            $o .= $renderer->render(new setask_header($this->setaskment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->setaskment->get_course_module()->id,
                                                            get_string('uploadgrades', 'setaskfeedback_offline')));
            $o .= $renderer->render(new setask_form('batchuploadfiles', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function download_grades() {
        ...
        $filename = clean_filename(get_string('offlinegradingworksheet', 'setaskfeedback_offline') . '-' .
                                   $this->setaskment->get_course()->shortname . '-' .
                                   $this->setaskment->get_instance()->name . '-' .
                                   $groupname .
                                   $this->setaskment->get_course_module()->id);
    ...
    public function get_grading_actions() {
        return array('uploadgrades'=>get_string('uploadgrades', 'setaskfeedback_offline'),
                    'downloadgrades'=>get_string('downloadgrades', 'setaskfeedback_offline'));
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $gradingmanager = get_grading_manager($this->setaskment->get_context(), 'mod_setask', 'submissions');
