 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setaskfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('setask');

        // Visible elements.
        $setaskment = $params['setaskment'];
        ...
        $scaleoptions = null;
        if ($setaskment->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id'=>-($setaskment->get_instance()->grade)))) {
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'setaskfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($setaskment->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'setask') . $setaskment->get_uniqueid_for_user($user->id);
            }

            $usergrade = $setaskment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($setaskment->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($setaskment->get_instance()->grade > -1) &&
                      (($grade < 0) || ($grade > $setaskment->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'setaskfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'setaskfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'setaskfeedback_offline'));
        }

        $mform->addElement('hidden', 'id', $setaskment->get_course_module()->id);
