...
require_once($CFG->dirroot . '/mod/setask/spherelib.php');
...
class setask {
    ...
    /** @var stdClass the setaskment record that contains the global settings for this setask instance */
    private $instance;

    /** @var stdClass the grade_item record for this setask instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this setask instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this setask instance belongs to */
    private $course;

    /** @var stdClass the admin config for all setask instances  */
    private $adminconfig;

    /** @var setask_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this setask instance */
    ...
    /** @var array cached list of participants for this setaskment. The cache key will be group, showactive and the context id */
    ...
    /**
     * Constructor for the base setask class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('setasksubmission');
        $this->feedbackplugins = $this->load_plugins('setaskfeedback');
    }
    ...
    /**
     * Based on the current setaskment settings should we display the intro.
     *
    ...
    /**
     * Does an setaskment have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype setasksubmission | setaskfeedback
     * @param string $type
     * @return mixed setask_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('setask'));
     ...
     * @return mixed setask_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('setaskfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed setask_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('setasksubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('setask'));
                $pluginclass = 'setask_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof setask_plugin) {
    ...
    /**
     * Display the setaskment, used by view.php
     *
     * The setaskment is displayed differently depending on your role,
     * the settings for the setaskment and the status of the setaskment.
     *
    ...
    public function view($action='') {
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/setask/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'setask');
        ...
        } else if ($action == 'viewpluginsetaskfeedback') {
            $o .= $this->view_plugin_content('setaskfeedback');
        } else if ($action == 'viewpluginsetasksubmission') {
            $o .= $this->view_plugin_content('setasksubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'setask'), $notices);
        } else {
     ...
     *             when upgrading an old setaskment to a new one (the plugins get called manually)
    ...
    /**
     * Delete all grades from the gradebook for this setaskment.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/setask',
                               $this->get_course()->id,
                               'mod',
                               'setask',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this setaskment.
        $fs = get_file_storage();
        ...
        $DB->delete_records('setask_submission', array('setaskment' => $this->get_instance()->id));
        $DB->delete_records('setask_grades', array('setaskment' => $this->get_instance()->id));
        $DB->delete_records('setask_plugin_config', array('setaskment' => $this->get_instance()->id));
        $DB->delete_records('setask_user_flags', array('setaskment' => $this->get_instance()->id));
        $DB->delete_records('setask_user_mapping', array('setaskment' => $this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('setask', array('id'=>$this->get_instance()->id));
     ...
     * setaskment submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'setask');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_setask_submissions)) {
            // Delete files associated with this setaskment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'setask'),
            ...
            $setaskids = $DB->get_records('setask', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($setaskids));

            $DB->delete_records_select('setask_submission', "setaskment $sql", $params);
            $DB->delete_records_select('setask_user_flags', "setaskment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('setask_grades', "setaskment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/setask/lib.php');
                setask_reset_gradebook($data->courseid);
                ...
                if ($this->get_instance()->blindmarking && $this->get_instance()->revealidentities) {
                    $DB->set_field('setask', 'revealidentities', 0, array('id' => $this->get_instance()->id));
                }
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('setask',
     ...
     *
     * @param setask_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(setask_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this setaskment.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this setaskment
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/setask/lib.php');
        $setask = clone $this->get_instance();
        $setask->cmidnumber = $coursemoduleid;

        // Set setask gradebook feedback plugin status (enabled and visible).
        $setask->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return setask_grade_item_update($setask, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('setask');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this setaskment.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'setask', 'instance'=>$instance->id);
            ...
            $DB->delete_records('event', array('modulename'=>'setask', 'instance'=>$instance->id));
        }
    }
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param setask_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(setask_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'setask'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'setask'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'setask'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'setask'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'setask');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'setask');
        return self::$modulenameplural;
    }

    /**
     * Has this setaskment been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this setaskment
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('setask', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the setaskment class. ' .
                                       'Cannot load the setaskment record.');
        }
        return $this->instance;
    }

    /**
     * Get the primary grade item for this setask instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    public function list_participants($currentgroup, $idsonly) {
        ...
        if (!isset($this->participants[$key])) {
            $users = get_enrolled_users($this->context, 'mod/setask:submit', $currentgroup, 'u.*', null, null, null,
    ...
    /**
     * Load a count of valid teams for this setaskment.
     *
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group setaskment because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/setask:submit', $currentgroup, true);

        $params['setaskid'] = $this->get_instance()->id;
    ...
    public function count_submissions($includenew = false) {
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/setask:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status) {
        ...
        $params['setaskid'] = $this->get_instance()->id;
        $params['setaskid2'] = $this->get_instance()->id;
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'setaskment_');
        // Zip files.
    ...
    /**
     * Finds all setaskment notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastcron = $DB->get_field('modules', 'lastcron', array('name' => 'setask'));

        // Collect all submissions that require mailing.
        // Submissions are included if all are true:
        //   - The setaskment is visible in the gradebook.
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {setask} a
                 JOIN {setask_grades} g ON g.setaskment = a.id
            LEFT JOIN {setask_user_flags} uf ON uf.setaskment = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'setask'
        ...
        if (!empty($submissions)) {
            ...
            mtrace('Processing ' . count($submissions) . ' setaskment submissions ...');
            ...
            foreach ($submissions as $submission) {
                ...
                mtrace("Processing setaskment submission $submission->id ...");
                ...
                if (!$cm->uservisible) {
                    // Hold mail notification for setaskments the user cannot access until later.
                    continue;
                }

                // Need to send this to the student.
                $messagetype = 'feedbackavailable';
                $eventtype = 'setask_notification';
                $updatetime = $submission->lastmodified;
                $modulename = get_string('modulename', 'setask');

                $uniqueid = 0;
                if ($submission->blindmarking && !$submission->revealidentities) {
                    $uniqueid = self::get_uniqueid_for_user_static($submission->setaskment, $user->id);
                }
                $showusers = $submission->blindmarking && !$submission->revealidentities;
                self::send_setaskment_notification($grader,
                                                   $user,
                                                   $messagetype,
                                                   $eventtype,
                                                   $updatetime,
                ...
                $flags = $DB->get_record('setask_user_flags', array('userid'=>$user->id, 'setaskment'=>$submission->setaskment));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('setask_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->setaskment = $submission->setaskment;
                    $flags->mailed = 1;
            ...
            mtrace('Done processing ' . count($submissions) . ' setaskment submissions');
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('setask', $record->id, 0, false, MUST_EXIST);
    ...
    /**
     * Update user flags for this user in this setaskment.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->setaskment <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('setask_user_flags', $flags);
        return $result;
    }
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/setask/extensionform.php');
        ...
        $header = new setask_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'setask'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_setask_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new setask_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the setaskment.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'setask') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('setaskment'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        if ($create) {
            ...
            $submission->setaskment = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('setask_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('setask_submission', 'latest', 0, $params);
            }
            $submission->status = ASSIGN_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('setask_submission', $submission);
            return $DB->get_record('setask_submission', array('id' => $sid));
        }
        return false;
    }

    /**
     * View a summary listing of all setaskments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('setask', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new setask_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['setask'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/setask:grade', $context)) {
                $submitted = $setaskment->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/setask:submit', $context)) {
                $usersubmission = $setaskment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'setask');
                } else {
                    $submitted = get_string('submissionstatus_', 'setask');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'setask', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_setask_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team setaskments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'setasksubmission') {
            ...
            $o .= $this->get_renderer()->render(new setask_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new setask_submission_plugin_submission($plugin,
                                                              $item,
                                                              setask_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_setask\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new setask_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new setask_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              setask_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_setask\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/setask:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('setask_portfolio_caller', $portfolioparams, 'mod_setask');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new setask_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'setask')));
        $gradingresult = new setask_gradingmessage(get_string('savegradingresult', 'setask'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new setask_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'setask')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new setask_gradingmessage(get_string('quickgradingresult', 'setask'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_submission($userid) {
        if (!$this->can_view_submission($userid)) {
            throw new required_capability_exception($this->context, 'mod/setask:viewgrades', 'nopermission', '');
        }
    }

    /**
     * Throw an error if the permissions to view grades in this setaskment are missing.
     *
     * @throws required_capability_exception
     * @return none
     */
    ...
    /**
     * Does this user have view grade or grade permission for this setaskment?
     *
     * @return bool
     */
    public function can_view_grades() {
        // Permissions check.
        if (!has_any_capability(array('mod/setask:viewgrades', 'mod/setask:grade'), $this->context)) {
            return false;
        }
    ...
    /**
     * Does this user have grade permission for this setaskment?
     *
     * @return bool
     */
    public function can_grade() {
        // Permissions check.
        if (!has_capability('mod/setask:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all setaskment submissions.
     *
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/setask:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'setask') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'setask'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new setask_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'setask'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'setask'));
            $url = new moodle_url('/mod/setask/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_setask\event\all_submissions_downloaded::create_from_setask($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_setask add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_setask');
        return $this->output;
    }
     ...
     *
     * For team setaskments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('setaskment'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('setaskment'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('setask_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('setaskment'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('setask_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->setaskment = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('setask_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('setask_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->setaskment   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('setask_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('setaskment'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('setask_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        // Need submit permission to submit an setaskment.
        require_capability('mod/setask:grade', $this->context);

        $header = new setask_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'setask'));
        $o .= $this->get_renderer()->render($header);
        ...
        $cache = cache::make_from_params(cache_store::MODE_SESSION, 'mod_setask', 'useridlist');
        if (!$userid) {
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new setask_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             setask_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             false,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'setask', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_setask_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new setask_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new setask_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
        ...
        \mod_setask\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/setask:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/setask/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/setask/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'setask'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_setask\event\reveal_identities_confirmation_page_viewed::create_from_setask($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/setask/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this setaskment.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/setask/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/setask/quickgradingform.php');
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'setask');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/setask/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'setask');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/setask:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/setask/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'setask');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/setask/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=setaskfeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'setask'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_setask', 'submissions');

        $perpage = (int) get_user_preferences('setask_perpage', 10);
        $filter = get_user_preferences('setask_filter', '');
        $markerfilter = get_user_preferences('setask_markerfilter', '');
        $workflowfilter = get_user_preferences('setask_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('setask_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/setask:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/setask:grade', '', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'setask');
            $markingallocationoptions[ASSIGN_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'setask');
        ...
        if ($markingworkflow) {
            ...
            $markingworkflowoptions[''] = get_string('filternone', 'setask');
        ...
        $gradingoptionsform = new mod_setask_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_setask_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new setask_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_setask_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new setask_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new setask_grading_table($this, $perpage, $filter, 0, false);
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $setaskform = new setask_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($setaskform);
        }
        $setaskform = new setask_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_setask.init_grading_options');
        $o .= $this->get_renderer()->render($setaskform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        \mod_setask\event\grading_table_viewed::create_from_setask($this)->trigger();
    ...
    /**
     * Message for students when setaskment submissions have been closed.
     *
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/setask/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/setask:viewblinddetails', $this->get_context());
            if ($hasviewblind) {
                return fullname($user);
            } else {
                $uniqueid = $this->get_uniqueid_for_user($user->id);
                return get_string('participant', 'setask') . ' ' . $uniqueid;
            }
        } else {
            return fullname($user);
        }
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/setask/submission_form.php');
        // Need submit permission to submit an setaskment.
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/setask:submit', $this->context);
            $title = get_string('editsubmission', 'setask');
        } else {
            ...
            $title = get_string('editsubmissionother', 'setask', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'setask'));
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_setask_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new setask_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_setask\event\submission_form_viewed::create_from_user($this, $user)->trigger();

        return $o;
    }

    /**
     * See if this setaskment has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/setask:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_setask_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/setask/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            if ($usercount >= 5) {
                $usershtml .= get_string('moreusers', 'setask', count($userlist) - 5);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usershtml .= $this->get_renderer()->render(new setask_user_summary($user,
                                                                $this->get_course()->id,
        ...
        $mform = new mod_setask_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new setask_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'setask'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new setask_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_setask\event\batch_set_workflow_state_viewed::create_from_setask($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/setask/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql();
        $markers = get_users_by_capability($this->get_context(), 'mod/setask:grade', '', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_setask_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new setask_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'setask'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new setask_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_setask\event\batch_set_marker_allocation_viewed::create_from_setask($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/setask/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_setask_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new setask_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_setask\event\submission_confirmation_form_viewed::create_from_setask($this)->trigger();
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_setask', 'submissions');
            ...
            $submissionstatus = new setask_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              setask_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
                                                              ...
                                                              $gradingstatus,
                                                              $instance->preventsubmissionnotingroup);
            if (has_capability('mod/setask:submit', $this->get_context(), $user)) {
                $o .= $this->get_renderer()->render($submissionstatus);
            }
            ...
            $cangrade = has_capability('mod/setask:grade', $this->get_context());
            ...
            if ((!is_null($gradebookgrade->grade) || !$emptyplugins)
                ...
                $feedbackstatus = new setask_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // The setaskment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('setask_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('setask_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_setask', ASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new setask_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new setask_plugin_header($plugin));
            }
        }
        ...
        if ($this->can_view_grades()) {
            ...
            $currenturl = new moodle_url('/mod/setask/view.php', array('id' => $this->get_course_module()->id));
            ...
            if ($instance->teamsubmission) {
                ...
                $summary = new setask_grading_summary($this->count_teams($activitygroup),
                ...
                $summary = new setask_grading_summary($countparticipants,
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        \mod_setask\event\submission_status_viewed::create_from_setask($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        $setask = clone $this->get_instance();
        $setask->cmidnumber = $this->get_course_module()->idnumber;
        // Set setask gradebook feedback plugin status (enabled and visible).
        $setask->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return setask_grade_item_update($setask, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this setaskment qualifies as submitted or draft.
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_REOPENED) {
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('setask_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $result = $DB->update_record('setask_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('setask_submission', $submission) && $result;
        }
    ...
    /**
     * Is this setaskment open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the setaskment locked?
     *
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the setaskment.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->setask_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid &&
                ...
                has_capability('mod/setask:submit', $this->context, $graderid)) {
            // User can edit their own submission.
            return true;
        }

        if (!has_capability('mod/setask:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/setask:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/setask:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($setaskmentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'setask', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/setask/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/setask/view.php?id=' . $coursemodule->id . '">' .
                     format_string($setaskmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'setask', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_setaskment_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'setask') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'setask');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->setaskment = format_string($setaskmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/setask/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'setask', $info);
        ...
        $eventdata->component       = 'mod_setask';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->setaskment;
    ...
    public function send_notification($userfrom,
                                      ...
                                      $eventtype,
                                      $updatetime) {
        self::send_setaskment_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'setask');
            return false;
        }
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_setask\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_setask\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an setaskment.
        require_capability('mod/setask:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/setask:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_setask\event\extension_granted::create_from_setask($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_extension(& $mform) {
        ...
        $mform = new mod_setask_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'setask');
        }
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified, uf.workflowstate, uf.allocatedmarker
                ...
                LEFT JOIN {setask_grades} g ON
                    u.id = g.userid AND
                    g.setaskment = :setaskid2 AND
                    g.attemptnumber = gmx.maxattempt
                LEFT JOIN {setask_user_flags} uf ON uf.setaskment = g.setaskment AND uf.userid = g.userid
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'setask');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('setaskfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_setask\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/setask',
                                          $this->course->id,
                                          'mod',
                                          'setask',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'setask');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/setask:revealidentities', $this->context);

        if ($this->get_instance()->revealidentities || empty($this->get_instance()->blindmarking)) {
            return false;
        }

        // Update the setaskment record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('setaskfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('setask_grades', array('setaskment'=>$this->get_instance()->id));
        ...
        \mod_setask\event\identities_revealed::create_from_setask($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_setask_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('setask_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('setask_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('setask_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('setask_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('setask_quickgrading', isset($formdata->quickgrading));
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'setask', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'setask');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'setask');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'setask');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'setask', $params) . ' <br>';
    ...
    /**
     * Copy the current setaskment submission from the last submitted attempt.
     *
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     * @return bool
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != ASSIGN_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'setask');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'setask');
            return false;
        }
        ...
        \mod_setask\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted setaskments
            // and they haven't been notified.
            \mod_setask\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    /**
     * Save setaskment submission for the current user.
     *
     * @param  stdClass $data
     * @param  array $notices Any error messages that should be shown
     *                        to the user.
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'setask');
            return true;
        }
        ...
        if ($pluginerror || $allempty) {
            if ($allempty) {
                $notices[] = get_string('submissionempty', 'mod_setask');
            }
            return false;
        }
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        require_sesskey();
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('duedatereached', 'setask');
            return false;
        }
        $instance = $this->get_instance();
     ...
     * This is specific to the setaskment, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'setask', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'setask');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $mform->addElement('hidden', 'grade', $name);
                    $mform->hardFreeze('grade');
                    $mform->setType('grade', PARAM_RAW);
                    $strgradelocked = get_string('gradelocked', 'setask');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'setask');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'setask')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'setask'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'setask');
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/setask:grade', '', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'setask'));
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'setask'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'setask');
        ...
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'setask'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'setask', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'setask'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'setask'));
        ...
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != ASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'setask'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'setask');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'setask'), $attemptreopenmethod);
            ...
            if ($maxattempts == ASSIGN_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'setask');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'setask'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'setask'), $attemptnumber + 1);
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'setask'));
                $mform->setDefault('addattempt', 0);
            }
        }
        $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'setask'));
        // Get setaskment visibility information for student.
        $modinfo = get_fast_modinfo($settings->course, $userid);
        $cm = $modinfo->get_cm($this->get_course_module()->id);

        // Don't allow notification to be sent if the student can't access the setaskment,
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'setask');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'setask');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'setask');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'setask');
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_setask\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_setask\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function process_set_batch_marking_workflow_state() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                // notified previously, the student can access the setaskment, and the state
                // is "Released".
                ...
                // Will not apply update if user does not have permission to setask this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    if ($state == ASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
                        // Update Gradebook.
                        $setask = clone $this->get_instance();
                        $setask->cmidnumber = $this->get_course_module()->idnumber;
                        // Set setask gradebook feedback plugin status.
                        $setask->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                        setask_update_grades($setask, $userid);
                    }

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_setask\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_setask\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_setask\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_setask\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
     ...
     * @return bool
     */
    public static function can_upgrade_setaskment($type, $version) {
        $setaskment = new setask(null, null, null);
        return $setaskment->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an setaskment from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old setaskment files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old setaskment context id
     * @param int $oldcomponent The old setaskment component ('setaskment')
     * @param int $oldfilearea The old setaskment filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new setaskment context id
     * @param int $newcomponent The new setaskment component ('setaskment')
     * @param int $newfilearea The new setaskment filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group setaskment
     * at most 1 time.
     *
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $setaskmentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('setaskfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('setaskid1'=>$setaskmentid,
                        'setaskid2'=>$setaskmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN {setask_submission} s
                                                    ON u.id = s.userid and s.setaskment = :setaskid1 AND
                                                    s.latest = 1
                                                JOIN {setask_grades} g
                                                    ON u.id = g.userid and g.setaskment = :setaskid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - setask them a random id.
     *
     * @param int $setaskid The setaskid to lookup
     */
    public static function allocate_unique_ids($setaskid) {
        global $DB;

        $cm = get_coursemodule_from_instance('setask', $setaskid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/setask:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('setask_user_mapping',
                                      array('setaskment'=>$setaskid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->setaskment = $setaskid;
                $record->userid = $user->id;
                $DB->insert_record('setask_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this setaskment.
     *
     * @param int $setaskid The setaskment id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($setaskid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('setaskment'=>$setaskid, 'userid'=>$userid);
        if ($record = $DB->get_record('setask_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($setaskid);
        ...
        return $DB->insert_record('setask_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this setaskment.
     *
     * @param int $setaskid The id of the setaskment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($setaskid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('setask_user_mapping',
                                      array('setaskment'=>$setaskid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/setask:grade', $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'setask');
            $states[ASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'setask');
        }
        if (has_any_capability(array('mod/setask:reviewgrades',
                                     'mod/setask:managegrades'), $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'setask');
            $states[ASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'setask');
        }
        if (has_any_capability(array('mod/setask:releasegrades',
                                     'mod/setask:managegrades'), $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'setask');
        }
...
/**
 * Portfolio caller class for mod_setask.
 *
 * @package   mod_setask
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the setaskment we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('setask', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the setaskment type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_setask');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'setaskment.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'setask' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return setask_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        $setaskment = new setask($context, null, null);
        return $setaskment->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/setask:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'setask');
    }
