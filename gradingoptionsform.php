 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_setask_grading_options_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('gradingoptions', 'setask'));
        // Visible elements.
        $options = array(-1=>get_string('all'), 10=>'10', 20=>'20', 50=>'50', 100=>'100');
        $mform->addElement('select', 'perpage', get_string('setaskmentsperpage', 'setask'), $options, $dirtyclass);
        $options = array('' => get_string('filternone', 'setask'),
                         ASSIGN_FILTER_NOT_SUBMITTED => get_string('filternotsubmitted', 'setask'),
                         ASSIGN_FILTER_SUBMITTED => get_string('filtersubmitted', 'setask'),
                         ASSIGN_FILTER_REQUIRE_GRADING => get_string('filterrequiregrading', 'setask'));
        if ($instance['submissionsenabled']) {
            $mform->addElement('select', 'filter', get_string('filter', 'setask'), $options, $dirtyclass);
        }
        if (!empty($instance['markingallocationopt'])) {
            $markingfilter = get_string('markerfilter', 'setask');
        ...
        if (!empty($instance['markingworkflowopt'])) {
            $workflowfilter = get_string('workflowfilter', 'setask');
            $mform->addElement('select', 'workflowfilter', $workflowfilter, $instance['markingworkflowopt'], $dirtyclass);
        }
        // Quickgrading.
        if ($instance['showquickgrading']) {
            $mform->addElement('checkbox', 'quickgrading', get_string('quickgrading', 'setask'), '', $dirtyclass);
            $mform->addHelpButton('quickgrading', 'quickgrading', 'setask');
        ...
        // Buttons.
        $this->add_action_buttons(false, get_string('updatetable', 'setask'));
    }
}
