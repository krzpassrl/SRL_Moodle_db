...
/**
 * Extends table_sql to provide a table of setaskment submissions
 *
 * @package   mod_setask
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_grading_table extends table_sql implements renderable {
    /** @var setask $setaskment */
    private $setaskment = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the setaskment class that is displaying this table
     *
     * @param setask $setaskment The setaskment class
     ...
     */
    public function __construct(setask $setaskment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_setask_grading');
        $this->is_persistent(true);
        $this->setaskment = $setaskment;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/setask:grantextension',
                                                  $this->setaskment->get_context());
        $this->hasgrade = $this->setaskment->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/setask:viewblinddetails',
                $this->setaskment->get_context());
        ...
        $urlparams = array('action'=>'grading', 'id'=>$setaskment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/setask/view.php', $urlparams);
        ...
        $users = array_keys( $setaskment->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['setaskmentid1'] = (int)$this->setaskment->get_instance()->id;
        $params['setaskmentid2'] = (int)$this->setaskment->get_instance()->id;
        $params['setaskmentid3'] = (int)$this->setaskment->get_instance()->id;

        $extrauserfields = get_extra_user_fields($this->setaskment->get_context());
        ...
        $from = '{user} u
                         LEFT JOIN {setask_submission} s
                                ON u.id = s.userid
                               AND s.setaskment = :setaskmentid1
                               AND s.latest = 1
                         LEFT JOIN {setask_grades} g
                                ON u.id = g.userid
                               AND g.setaskment = :setaskmentid2 ';

        // For group submissions we don't immediately create an entry in the setask_submission table for each user,
        // instead the userid is set to 0. In this case we use a different query to retrieve the grade for the user.
        if ($this->setaskment->get_instance()->teamsubmission) {
            $params['setaskmentid4'] = (int) $this->setaskment->get_instance()->id;
            $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                                  FROM {setask_grades} mxg
                                 WHERE mxg.setaskment = :setaskmentid4
                              GROUP BY mxg.userid';
        ...
        $from .= 'LEFT JOIN {setask_user_flags} uf
                         ON u.id = uf.userid
        ...
        if ($this->setaskment->is_any_submission_plugin_enabled()) {
        ...
        if ($this->setaskment->get_instance()->markingworkflow &&
            $this->setaskment->get_instance()->markingallocation) {
            if (has_capability('mod/setask:manageallocations', $this->setaskment->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('setask_markerfilter', '');
            ...
            if (!empty($workflowstates)) {
                ...
                if ($workflowfilter == ASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('setask_user_flags', 'workflowstate', true, true).')';
        ...
        // User picture.
        if ($this->hasviewblind || !$this->setaskment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'setask');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'setask');

        // Team submission columns.
        if ($setaskment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'setask');
        }
        // Allocated marker.
        if ($this->setaskment->get_instance()->markingworkflow &&
            $this->setaskment->get_instance()->markingallocation &&
            has_capability('mod/setask:manageallocations', $this->setaskment->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'setask');
        }
        // Grade.
        $columns[] = 'grade';
        $headers[] = get_string('grade');
        if ($this->is_downloading()) {
            if ($this->setaskment->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'setask');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'setask');
            }

            if ($this->setaskment->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'setask');
            }
            // Add a column for the list of valid marking workflow states.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'setask');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        // Submission plugins.
        if ($setaskment->is_any_submission_plugin_enabled()) {
            $columns[] = 'timesubmitted';
            $headers[] = get_string('lastmodifiedsubmission', 'setask');

            foreach ($this->setaskment->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'setask');
        ...
        $this->gradinginfo = grade_get_grades($this->setaskment->get_course()->id,
                                              'mod',
                                              'setask',
                                              $this->setaskment->get_instance()->id,
                                              $users);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this setaskment.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'setask') .
               $this->setaskment->get_uniqueid_for_user($row->userid);
    }
    ...
    public function col_workflowstate($row) {
        ...
        return get_string('markingworkflowstate' . $state, 'setask');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($this->quickgrading && has_capability('mod/setask:manageallocations', $this->setaskment->get_context()) &&
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->setaskment->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->setaskment->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->setaskment->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->setaskment->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        return get_string('defaultteam', 'setask');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->setaskment->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->setaskment->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            $courseid = $this->setaskment->get_course()->id;
            $link= new moodle_url('/user/view.php', array('id' =>$row->id, 'course'=>$courseid));
            $fullname = $this->output->action_link($link, $this->setaskment->fullname($row));
        } else {
            $fullname = $this->setaskment->fullname($row);
        }

        if (!$this->setaskment->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'setask', $this->setaskment->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this setaskment.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->setaskment->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            $name = $this->setaskment->fullname($row);
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'setask', $name),
                                            'mod_setask');
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->setaskment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $status, 'setask'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'setask');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'setask'), 'submissiongraded');
                } else if (!$timesubmitted) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'setask', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'setask', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'setask');
        }
        ...
        $submissionsopen = $this->setaskment->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        $caneditsubmission = $this->setaskment->can_edit_submission($row->id, $USER->id);

        // Hide for offline setaskments.
        if ($this->setaskment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == ASSIGN_SUBMISSION_STATUS_DRAFT ||
                    !$this->setaskment->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'setask');
                    ...
                    $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/setask/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'setask');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/setask/view.php', $urlparams);
                $description = get_string('editsubmission', 'setask');
        ...
        if (($this->setaskment->get_instance()->duedate ||
                $this->setaskment->get_instance()->cutoffdate) &&
             ...
             $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/setask/view.php', $urlparams);
             $description = get_string('grantextension', 'setask');
        ...
        if ($row->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED &&
            ...
            $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/setask/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'setask');
        ...
        if ($row->status == ASSIGN_SUBMISSION_STATUS_DRAFT &&
            ...
            $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/setask/view.php', $urlparams);
            $description = get_string('submitforgrading', 'setask');
        ...
        $ismanual = $this->setaskment->get_instance()->attemptreopenmethod == ASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != ASSIGN_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->setaskment->get_instance()->maxattempts == ASSIGN_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->setaskment->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->setaskment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/setask/view.php', $urlparams);
            $description = get_string('addattempt', 'setask');
     ...
     */
    private function format_plugin_summary_with_link(setask_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
            ...
            if ($plugin->get_subtype() == 'setasksubmission') {
                if ($this->setaskment->get_instance()->teamsubmission) {
                    ...
                    if ($row->status == ASSIGN_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->setaskment->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->setaskment = $this->setaskment->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->setaskment = $this->setaskment->get_instance()->id;
     ...
     *
     * @return string the setaskment name
     */
    public function get_setaskment_name() {
        return $this->setaskment->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->setaskment->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->setaskment->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->setaskment->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this setaskment accept submissions
     */
    public function submissions_enabled() {
        return $this->setaskment->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->setaskment->get_course_context();
