 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_submission_comments extends setask_submission_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'setasksubmission_comments');
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        $o = $this->setaskment->get_renderer()->container($comment->output(true), 'commentscontainer');
        return $o;
    }
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldsetaskment, & $log) {
        if ($oldsetaskment->setaskmenttype == 'upload') {
            // Disable if allow notes was not enabled.
            if (!$oldsetaskment->var2) {
                $this->disable();
            }
        }
        return true;
