 ...
 * @return bool
 */
function setasksubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('setask_submission', array('id'=>$options->itemid))) {
    ...
    if ($setaskment->get_instance()->id != $submission->setaskment) {
        throw new comment_exception('invalidcontext');
    }
    $canview = false;
    if ($submission->userid) {
        $canview = $setaskment->can_view_submission($submission->userid);
    } else {
        $canview = $setaskment->can_view_group_submission($submission->groupid);
    }
    if (!$canview) {
 ...
 * @return array
 */
function setasksubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($setaskment->get_instance()->teamsubmission &&
        !$setaskment->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$setaskment->get_instance()->teamsubmission &&
        !$setaskment->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function setasksubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($setaskment->is_blind_marking() && !empty($comments)) {
        // Blind marking is being used, may need to map unique anonymous ids to the comments.
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'setask'));
        ...
        foreach ($comments as $comment) {
            ...
            if (empty($usermappings[$comment->userid])) {
                ...
                $anonid = $setaskment->get_uniqueid_for_user($comment->userid);
                ...
                $usermappings[$comment->userid]->avatar = $setaskment->get_renderer()->user_picture($commenter,
 ...
 */
function setasksubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('setask_submission', array('id' => $submissionid));
