...
class setasksubmission_file_events_testcase extends advanced_testcase {

    /** @var stdClass $user A user to submit an setaskment. */
    protected $user;

    /** @var stdClass $course New course created to hold the setaskment activity. */
    protected $course;

    /** @var stdClass $cm A context module object. */
    protected $cm;

    /** @var stdClass $context Context of the setaskment activity. */
    protected $context;

    /** @var stdClass $setask The setaskment object. */
    protected $setask;

    /** @var stdClass $files Files that are being submitted for the setaskment. */
    protected $files;
    ...
    /**
     * Setup all the various parts of an setaskment activity including creating a file submission.
     */
    protected function setUp() {
        ...
        $this->cm = get_coursemodule_from_instance('setask', $instance->id);
        $this->context = context_module::instance($this->cm->id);
        $this->setask = new testable_setask($this->context, $this->cm, $this->course);

        $this->setUser($this->user->id);
        $this->submission = $this->setask->get_user_submission($this->user->id, true);
        ...
        $dummy = (object) array(
            ...
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'mysetaskmnent.png'
        );
        $this->fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $this->files = $fs->get_area_files($this->context->id, 'setasksubmission_file', ASSIGNSUBMISSION_FILE_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $data = new stdClass();
        $plugin = $this->setask->get_submission_plugin_by_type('file');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\setasksubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\setasksubmission_file\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\setasksubmission_file\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
