...
class setasksubmission_onlinetext_events_testcase extends advanced_testcase {
    ...
    /** @var stdClass $data General data for the setaskment submission. */
    protected $data;

    /**
     * Setup all the various parts of an setaskment activity including creating an onlinetext submission.
     */
    ...
    public function test_assessable_uploaded() {
        ...
        $plugin = $this->setask->get_submission_plugin_by_type('onlinetext');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\setasksubmission_onlinetext\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $this->assertCount(2, $events);
        $event = $events[1];
        $this->assertInstanceOf('\setasksubmission_onlinetext\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        $event = $events[3];
        $this->assertInstanceOf('\setasksubmission_onlinetext\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
