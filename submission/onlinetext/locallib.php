...
// File area for online text submission setaskment.
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class setask_submission_onlinetext extends setask_submission_plugin {
    ...
    public function get_name() {
        return get_string('onlinetext', 'setasksubmission_onlinetext');
    }
    ...
    private function get_onlinetext_submission($submissionid) {
        ...
        return $DB->get_record('setasksubmission_onlinetext', array('submission'=>$submissionid));
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $name = get_string('wordlimit', 'setasksubmission_onlinetext');

        // Create a text box that can be enabled/disabled for onlinetext word limit.
        $wordlimitgrp = array();
        $wordlimitgrp[] = $mform->createElement('text', 'setasksubmission_onlinetext_wordlimit', '', $options);
        $wordlimitgrp[] = $mform->createElement('checkbox', 'setasksubmission_onlinetext_wordlimit_enabled',
                '', get_string('enable'));
        $mform->addGroup($wordlimitgrp, 'setasksubmission_onlinetext_wordlimit_group', $name, ' ', false);
        $mform->addHelpButton('setasksubmission_onlinetext_wordlimit_group',
                              'wordlimit',
                              'setasksubmission_onlinetext');
        $mform->disabledIf('setasksubmission_onlinetext_wordlimit',
                           'setasksubmission_onlinetext_wordlimit_enabled',
                           'notchecked');

        // Add numeric rule to text field.
        $wordlimitgrprules = array();
        $wordlimitgrprules['setasksubmission_onlinetext_wordlimit'][] = array(null, 'numeric', null, 'client');
        $mform->addGroupRule('setasksubmission_onlinetext_wordlimit_group', $wordlimitgrprules);

        // Rest of group setup.
        $mform->setDefault('setasksubmission_onlinetext_wordlimit', $defaultwordlimit);
        $mform->setDefault('setasksubmission_onlinetext_wordlimit_enabled', $defaultwordlimitenabled);
        $mform->setType('setasksubmission_onlinetext_wordlimit', PARAM_INT);
        $mform->disabledIf('setasksubmission_onlinetext_wordlimit_group',
                           'setasksubmission_onlinetext_enabled',
                           'notchecked');
    }
    ...
    public function save_settings(stdClass $data) {
        if (empty($data->setasksubmission_onlinetext_wordlimit) || empty($data->setasksubmission_onlinetext_wordlimit_enabled)) {
            $wordlimit = 0;
            $wordlimitenabled = 0;
        } else {
            $wordlimit = $data->setasksubmission_onlinetext_wordlimit;
            $wordlimitenabled = 1;
        }
    ...
    private function get_edit_options() {
         $editoroptions = array(
           ...
           'maxbytes' => $this->setaskment->get_course()->maxbytes,
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        $params = array(
            'context' => context_module::instance($this->setaskment->get_course_module()->id),
        ...
        $event = \setasksubmission_onlinetext\event\assessable_uploaded::create($params);
        $event->trigger();
        ...
        if ($onlinetextsubmission) {
            ...
            $updatestatus = $DB->update_record('setasksubmission_onlinetext', $onlinetextsubmission);
            $event = \setasksubmission_onlinetext\event\submission_updated::create($params);
            $event->set_setask($this->setaskment);
            $event->trigger();
            return $updatestatus;
        } else {
            ...
            $onlinetextsubmission->setaskment = $this->setaskment->get_instance()->id;
            $onlinetextsubmission->id = $DB->insert_record('setasksubmission_onlinetext', $onlinetextsubmission);
            $params['objectid'] = $onlinetextsubmission->id;
            $event = \setasksubmission_onlinetext\event\submission_created::create($params);
            $event->set_setask($this->setaskment);
            $event->trigger();
    ...
    public function get_editor_fields() {
        return array('onlinetext' => get_string('pluginname', 'setasksubmission_comments'));
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($onlinetextsubmission) {
            $text = $this->setaskment->render_editor_content(ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
            ...
            if (!empty($CFG->enableplagiarism)) {
                ...
                $plagiarismlinks .= plagiarism_get_links(array('userid' => $submission->userid,
                    'content' => trim($text),
                    'cmid' => $this->setaskment->get_course_module()->id,
                    'course' => $this->setaskment->get_course()->id,
                    'setaskment' => $submission->setaskment));
            }
            if ($text != $shorttext) {
                $wordcount = get_string('numwords', 'setasksubmission_onlinetext', count_words($text));
    ...
    public function get_files(stdClass $submission, stdClass $user) {
        ...
        if ($onlinetextsubmission) {
            $finaltext = $this->setaskment->download_rewrite_pluginfile_urls($onlinetextsubmission->onlinetext, $user, $this);
            $formattedtext = format_text($finaltext,
                                         $onlinetextsubmission->onlineformat,
                                         array('context'=>$this->setaskment->get_context()));
            $head = '<head><meta charset="UTF-8"></head>';
            $submissioncontent = '<!DOCTYPE html><html>' . $head . '<body>'. $formattedtext . '</body></html>';

            $filename = get_string('onlinetextfilename', 'setasksubmission_onlinetext');
            $files[$filename] = array($submissioncontent);

            $fs = get_file_storage();

            $fsfiles = $fs->get_area_files($this->setaskment->get_context()->id,
                                           'setasksubmission_onlinetext',
                                           ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                           $submission->id,
                                           'timemodified',
    ...
    public function view(stdClass $submission) {
        ...
        if ($onlinetextsubmission) {
            ...
            $result .= $this->setaskment->render_editor_content(ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                                                $onlinetextsubmission->submission,
                                                                $this->get_type(),
                                                                'onlinetext',
                                                                'setasksubmission_onlinetext');
    ...
    /**
     * Return true if this plugin can upgrade an old Moodle 2.2 setaskment of this type and version.
     *
     ...
     * @param stdClass $oldsetaskment - the database for the old setaskment instance
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('setasksubmission_onlinetext', $onlinetextsubmission) > 0) {
            $log .= get_string('couldnotconvertsubmission', 'mod_setask', $submission->userid);
            return false;
        }
    ...
    public function delete_instance() {
        global $DB;
        $DB->delete_records('setasksubmission_onlinetext',
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'setasksubmission_onlinetext',
        ...
        // Copy the setasksubmission_onlinetext record.
    ...
    public function check_word_count($submissiontext) {
        ...
        if ($wordcount <= $wordlimit) {
            ...
            $errormsg = get_string('wordlimitexceeded', 'setasksubmission_onlinetext',
