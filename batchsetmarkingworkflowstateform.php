...
require_once($CFG->dirroot . '/mod/setask/feedback/file/locallib.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_setask_batch_set_marking_workflow_state_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $formheader = get_string('batchsetmarkingworkflowstateforusers', 'setask', $params['userscount']);

        $mform->addElement('header', 'general', $formheader);
        $mform->addElement('static', 'userslist', get_string('selectedusers', 'setask'), $params['usershtml']);

        $options = $params['markingworkflowstates'];
        $mform->addElement('select', 'markingworkflowstate', get_string('markingworkflowstate', 'setask'), $options);
